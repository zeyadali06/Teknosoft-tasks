import 'package:flutter/material.dart';

const Color kPrimaryColor = Color(0xfffccf2f);
const double kPadding = 20;
const double kRadius = 30;
const String loginStatusPrefKey = 'loginstatus';
const String emailPrefKey = 'email';
const String passwordPrefKey = 'password';
const String urlPrefKey = 'profileImageURL';
const String usersCollection = 'usersCollection';
const String favourateCollection = 'favourateCollection';
const String favouratesField = 'favourates';
const String cartCollection = 'cartCollection';
const String cartField = 'cart';
const String addressesCollection = 'addressesCollection';
const String addressesField = 'addresses';
const String profileImagesFolderPath = 'profil_images';
const String defaultProfileImageURL =
    "https://firebasestorage.googleapis.com/v0/b/ecommerce-app-developmen-47370.appspot.com/o/DefaultProfileImage.png?alt=media&token=6a3b2885-e396-4af9-8817-6cb5753614b7";
